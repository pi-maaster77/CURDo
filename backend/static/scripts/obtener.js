    ...
    .then(data => {
        data.forEach(elemento => {
            ...
            fila.innerHTML = `
                ...
                <td><button onclick="eliminar(${elemento.id})">BotonEliminar</button></td>
                <td><button onclick="actualizar(${elemento.id})">Actualizar</button></td>
                <td>${elemento.tags.map(tag => tag.nombre).join(", ")}</td>
                <td><button onclick="gestionarTags(${elemento.id})">Gestionar Tags</button></td>
            `;
